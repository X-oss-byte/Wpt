...
def parseCodePoint(aHexaString):
    return int(f"0x{aHexaString[1:]}", 16)
...
for mathvariant in mathvariantTransforms:
    if mathvariant == "auto":
        continue
    font = mathfont.create(
        f"mathvariant-{mathvariant}", "Copyright (c) 2016 MathML Association"
    )
...
def generateTestFor(mathvariant, mathml):
    ...
    print(f"Generating tests for {mathvariant}...", end="")
    if mathml:
        reftest = open(f"../relations/css-styling/mathvariant-{mathvariant}.html", "w")
        reftestReference = open(
            f"../relations/css-styling/mathvariant-{mathvariant}-ref.html", "w"
        )
    else:
        reftest = open(
            f"../../css/css-text/text-transform/math/text-transform-math-{mathvariant}-001.html",
            "w",
        )
        reftestReference = open(
            f"../../css/css-text/text-transform/math/text-transform-math-{mathvariant}-001-ref.html",
            "w",
        )
    source = '\
<!DOCTYPE html>\n\
...
<title>%s</title>\n'
    if mathml:
        reftest.write(source % f"mathvariant {mathvariant}")
        reftestReference.write(source % f"mathvariant {mathvariant} (reference)")
    else:
        reftest.write(source % f"text-transform math-{mathvariant}")
        reftestReference.write(
            source % f"text-transform math-{mathvariant} (reference)"
        )
    if mathvariant == "auto":
        mathAssert = "Verify that a single-char <mi> is equivalent to an <mi> with the transformed italic unicode character."
        mapping = "italic"
    else:
        mathAssert = f"Verify that a single-char <mtext> with a {mathvariant} mathvariant is equivalent to an <mtext> with the transformed unicode character."
        mapping = mathvariant
    if mathml:
        source = '\
...
<meta name="assert" content="Verify that a character with \'text-transform: math-%s\' renders the same as the transformed unicode character.">\n'
    ...
    WOFFfont = f"mathvariant-{mapping}.woff"
    source = '\
<style>\n\
  @font-face {\n\
...
<body>\n\
  ...
  <p>Test passes if all the equalities below are true.</p>\n' % WOFFfont
    ...
    for charIndex, baseChar in enumerate(mathvariantTransforms[mathvariant], start=1):
        transformedChar = mathvariantTransforms[mathvariant][baseChar]
        if mathvariant == "auto":
