...
suffixes = [iter(SUFFIXES) for _ in range(6)]
...
def write_test_file(idx, suffix, wm, orient, value, position):
    ...
    write_file(
        ...
        TEST_TEMPLATE.format(
            ...
            index=idx,
            position=position,
            title=wm if orient == "mixed" else f"{wm}, {orient}",
        ),
    )
    reftest_items.append(f"== {filename} {REF_FILE.format(idx)}")
...
def write_test_files(wm, orient, pos1, pos2):
    ...
    write_test_file(idx, next(suffix), wm, orient, f"{pos1} {pos2}", position)
    write_test_file(idx, next(suffix), wm, orient, f"{pos2} {pos1}", position)
